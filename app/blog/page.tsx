...
export default async function BlogPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <main className={styles.page} id="top">
    ...
    {allPosts.length > 0 && filtered.length > 0 && featured && (
      <section className={styles.featured} id="stories">
        <Link href={`/blog/${featured.slug}`} className={styles.featuredImage} aria-label={`Read ${featured.title}`}>{featured.coverImageUrl?<Image src={featured.coverImageUrl} alt={featured.coverImageAlt || ""} fill priority sizes="(max-width: 800px) 100vw, 60vw"/>:<span className={styles.coverPlaceholder} aria-hidden="true"><i>TRC</i><em>Design · Build · Roof</em></span>}<span className={styles.imageCut}/><b>01</b></Link>
          ...
          {gridPosts.map((post, index) => {
            ...
            return (
              <Reveal as="article" key={post.id} className={styles.card} delayMs={Math.min(index, 5) * 60}>
                <Link href={`/blog/${post.slug}`} className={styles.cardImage}>{post.coverImageUrl?<Image src={post.coverImageUrl} alt={post.coverImageAlt || ""} fill sizes="(max-width: 700px) 100vw, (max-width: 1000px) 50vw, 33vw"/>:<span className={styles.coverPlaceholder} aria-hidden="true"><i>TRC</i><em>New story</em></span>}<span/><b>{number}</b></Link>
